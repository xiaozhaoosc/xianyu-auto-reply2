...
export const mainNavItems: NavEntry[] = [
  ...
  },
  {
    ...
    label: '线索池',
    children: [
      { key: 'lead-capture-overview', icon: LayoutDashboard, label: '线索总览', path: '/lead-capture/overview' },
      { key: 'lead-capture-comments', icon: MessageSquare, label: '意向评论', path: '/lead-capture/comments' },
      { key: 'lead-capture-items', icon: Package, label: '线索商品', path: '/lead-capture/items' },
      { key: 'lead-capture-tasks', icon: Timer, label: '采集任务', path: '/lead-capture/tasks' },
      { key: 'lead-capture-logs', icon: ScrollText, label: '扫描日志', path: '/lead-capture/logs' },
      { key: 'lead-capture-probe', icon: Search, label: '评论探针', path: '/lead-capture/probe' },
    ],
  },
